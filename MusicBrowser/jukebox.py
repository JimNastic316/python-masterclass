...
def get_albums(event):
    ...
    for row in conn.execute("SELECT albums.name FROM albums WHERE albums.artist = ? ORDER BY albums.name", artist_id):
        alist.append(row[0])
    albumLV.set(tuple(alist))  # convert alist from a list to a tuple
    songLV.set(("Choose and album",))   # clears the "Songs" box when a new Artist is selected


# Code below creates an error See comments for Sect 361 Show Songs from Album
def get_songs(event):
    lb = event.widget
...
# Populate artist listbox with artists from db
