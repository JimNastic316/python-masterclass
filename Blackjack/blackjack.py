...
def _deal_card(frame: object) -> object:
    # pop next card off deck
    next_card = deck.pop(0)  # Use '0' to pop from top of deck, if blank would be from the bottom
    # add it card back to the back, at the bottom of deck
...
def score_hand(hand):
    ...
    for next_card in hand:
        ...
        if card_value == 1 and not ace:
            ...
            card_value = 11
        score += card_value
        # If we would bust, check if there is an ace, and subtract 10 if can
        if score > 21 and ace:
            score -= 10
            ace = False
    return score
